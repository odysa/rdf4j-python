...
async def filter_repositories_by_access():
    ...
    async with AsyncRdf4j("http://localhost:19780/rdf4j-server") as db:
        ...
        read_only_repos = [repo for repo in repositories if repo.readable and not repo.writable]
...
async def search_repositories_by_name(search_term):
    ...
    async with AsyncRdf4j("http://localhost:19780/rdf4j-server") as db:
        ...
        matching_repos = [
            ...
            if search_term.lower() in repo.id.lower() or search_term.lower() in repo.title.lower()
        ]
...
async def display_repository_summary():
    ...
    async with AsyncRdf4j("http://localhost:19780/rdf4j-server") as db:
        ...
        read_only_count = sum(1 for repo in repositories if repo.readable and not repo.writable)
