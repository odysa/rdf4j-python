...
async def sample_data_repo(mem_repo: AsyncRdf4JRepository):
    ...
    people_data = [
        ...
        Quad(ex["alice"], ex["age"], Literal("30", datatype=XSD.integer), ex["people_graph"]),
        Quad(ex["alice"], ex["email"], Literal("alice@example.org"), ex["people_graph"]),
        ...
        Quad(ex["bob"], ex["age"], Literal("25", datatype=XSD.integer), ex["people_graph"]),
        ...
        Quad(ex["charlie"], ex["age"], Literal("45", datatype=XSD.integer), ex["people_graph"]),
        Quad(ex["charlie"], ex["email"], Literal("charlie@example.org"), ex["people_graph"]),
...
class TestSelectQueries:
    ...
    async def test_select_all_people(self, sample_data_repo):
        ...
        SELECT ?person ?name WHERE {
            ?person a <http://www.example.org/Person> .
            ?person <http://www.example.org/name> ?name .
        }
        """
    ...
    async def test_select_with_filter(self, sample_data_repo):
        ...
        SELECT ?person ?name ?age WHERE {
            ?person <http://www.example.org/name> ?name .
            ?person <http://www.example.org/age> ?age .
            FILTER(?age > 30)
        }
        """
        result = await sample_data_repo.query(query)
    ...
    async def test_select_with_optional(self, sample_data_repo):
        ...
        SELECT ?person ?name ?email WHERE {
            ?person <http://www.example.org/name> ?name .
            OPTIONAL { ?person <http://www.example.org/email> ?email }
        }
        """
        result = await sample_data_repo.query(query)
        assert isinstance(result, QuerySolutions)
        ...
        results_by_name = {solution["name"].value: solution for solution in results_list}
        
        assert "email" in results_by_name["Alice"]
        assert results_by_name["Alice"]["email"].value == "alice@example.org"
        
        assert "email" in results_by_name["Charlie"]
        assert results_by_name["Charlie"]["email"].value == "charlie@example.org"
        
        assert "email" not in results_by_name["Bob"] or results_by_name["Bob"]["email"] is None
    ...
    async def test_select_with_join(self, sample_data_repo):
        ...
        SELECT ?person ?name ?company WHERE {
            ?person <http://www.example.org/name> ?name .
            ?person <http://www.example.org/worksFor> ?org .
            ?org <http://www.example.org/name> ?company .
        }
        """
        result = await sample_data_repo.query(query)
        assert isinstance(result, QuerySolutions)
        ...
        results_by_person = {solution["name"].value: solution for solution in results_list}
...
class TestAskQueries:
    ...
    async def test_ask_person_exists(self, sample_data_repo):
        ...
        query = """
        ASK {
            ?person <http://www.example.org/name> "Alice" .
        }
        """
        result = await sample_data_repo.query(query)
    ...
    async def test_ask_person_not_exists(self, sample_data_repo):
        ...
        query = """
        ASK {
            ?person <http://www.example.org/name> "David" .
        }
        """
        result = await sample_data_repo.query(query)
        assert isinstance(result, QueryBoolean)
    ...
    async def test_ask_with_filter(self, sample_data_repo):
        ...
        result = await sample_data_repo.query(query)
        assert isinstance(result, QueryBoolean)
        assert result is True  # Charlie is 45
    ...
    async def test_ask_relationship_exists(self, sample_data_repo):
        ...
        query = """
        ASK {
            ?person1 <http://www.example.org/knows> ?person2 .
        }
        """
        result = await sample_data_repo.query(query)
        assert isinstance(result, QueryBoolean)
    ...
    async def test_ask_specific_relationship(self, sample_data_repo):
        ...
        query = """
        ASK {
            <http://www.example.org/alice> <http://www.example.org/knows> <http://www.example.org/bob> .
        }
        """
        result = await sample_data_repo.query(query)
        assert isinstance(result, QueryBoolean)
    ...
    async def test_ask_email_exists(self, sample_data_repo):
        ...
        query = """
        ASK {
            ?person <http://www.example.org/email> ?email .
        }
        """
        result = await sample_data_repo.query(query)
        assert isinstance(result, QueryBoolean)
...
class TestConstructQueries:
    ...
    async def test_construct_simplified_person_data(self, sample_data_repo):
        ...
        query = """
        CONSTRUCT {
            ?person <http://www.example.org/hasName> ?name ;
                    <http://www.example.org/hasAge> ?age .
        }
        WHERE {
            ?person <http://www.example.org/name> ?name .
            ?person <http://www.example.org/age> ?age .
    ...
    async def test_construct_employment_relationships(self, sample_data_repo):
        ...
        query = """
        CONSTRUCT {
            ?person <http://www.example.org/employedBy> ?companyName .
        }
        WHERE {
            ?person <http://www.example.org/worksFor> ?company .
            ?company <http://www.example.org/name> ?companyName .
        }
        """
        result = await sample_data_repo.query(query)
        assert isinstance(result, QueryTriples)
        ...
        for triple in triples_list:
            assert str(triple.predicate) == "http://www.example.org/employedBy"
        ...
        assert company_by_person["http://www.example.org/alice"] == "TechCorp"
        assert company_by_person["http://www.example.org/bob"] == "DataInc"
        assert company_by_person["http://www.example.org/charlie"] == "TechCorp"
    ...
    async def test_construct_with_filter(self, sample_data_repo):
        ...
        query = """
        CONSTRUCT {
            ?person <http://www.example.org/isSenior> true .
        }
        WHERE {
            ?person <http://www.example.org/name> ?name .
            ?person <http://www.example.org/age> ?age .
        ...
        assert str(triple.subject) == "http://www.example.org/charlie"
        assert str(triple.predicate) == "http://www.example.org/isSenior"
    ...
    async def test_construct_social_network(self, sample_data_repo):
        ...
        query = """
        CONSTRUCT {
            ?person1 <http://www.example.org/connected> ?person2 .
            ?person2 <http://www.example.org/connected> ?person1 .
        }
        WHERE {
            ?person1 <http://www.example.org/knows> ?person2 .
        }
        ...
        triples_list = list(result)
        assert len(triples_list) == 6  # 3 knows relationships * 2 directions each
        
        # Check that all triples use the connected predicate
        for triple in triples_list:
            assert str(triple.predicate) == "http://www.example.org/connected"
...
class TestDescribeQueries:
    ...
    async def test_describe_specific_person(self, sample_data_repo):
        ...
        query = """
        DESCRIBE <http://www.example.org/alice>
        """
        result = await sample_data_repo.query(query)
        assert isinstance(result, QueryTriples)
        ...
        alice_uri = IRI("http://www.example.org/alice")
        for triple in triples_list:
    ...
    async def test_describe_with_where(self, sample_data_repo):
        ...
        charlie_uri = IRI("http://www.example.org/charlie")
    ...
    async def test_describe_companies(self, sample_data_repo):
        ...
        DESCRIBE ?company WHERE {
            ?company a <http://www.example.org/Company> .
        }
        """
        result = await sample_data_repo.query(query)
        assert isinstance(result, QueryTriples)
        ...
        company_a_uri = IRI("http://www.example.org/company_a")
        company_b_uri = IRI("http://www.example.org/company_b")
...
class TestComplexQueries:
    ...
    async def test_complex_select_with_multiple_joins(self, sample_data_repo):
        ...
        SELECT ?personName ?companyName ?friendName WHERE {
            ?person <http://www.example.org/name> ?personName .
            ?person <http://www.example.org/worksFor> ?company .
            ?company <http://www.example.org/name> ?companyName .
            ?person <http://www.example.org/knows> ?friend .
            ?friend <http://www.example.org/name> ?friendName .
            ?person <http://www.example.org/age> ?age .
            FILTER(?age < 35)
        }
        ORDER BY ?personName
        ...
        expected_names = {"Alice"}  # Alice knows Bob and Charlie, and is under 35
    ...
    async def test_complex_construct_with_calculations(self, sample_data_repo):
        ...
        query = """
        CONSTRUCT {
            ?person <http://www.example.org/ageGroup> ?ageGroup .
            ?person <http://www.example.org/hasContact> true .
        }
        WHERE {
            ?person <http://www.example.org/name> ?name .
            ?person <http://www.example.org/age> ?age .
